...
def residues_graph(resultfile, client, crop, date = "all", hide = False, fsize=20): ## n.1
    ...
    for year in dates: # Will produce a graph for each date.
        ...
        if hide == True:
            ...
            writer = pd.ExcelWriter('{}\\Compound_index.xlsx'.format(choose_dr), engine='xlsxwriter')
...
def compound_per_client(resultfile, infofile, compound, crop, date ="all", hide=False, fsize=20): ## n.2 
    ...
    if hide == True:            
        ...
        choose_dr = askdirectory()
        writer = pd.ExcelWriter('{}\\Client_index.xlsx'.format(choose_dr), engine='xlsxwriter')
        data_client.to_excel(writer, sheet_name='Sheet1')
        writer.save()
    return(fig_list)
